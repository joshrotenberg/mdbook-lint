...
fn test_project_files_corpus() {
    ...
    let project_files = [
        ...
        "docs/src/mdbook-integration.md",
    ];

    for file in &project_files {
        // Try both workspace root and crate-relative paths
        let workspace_path = PathBuf::from(file);
        let crate_relative_path = PathBuf::from("../..").join(file);

        let (path, exists) = if workspace_path.exists() {
            (workspace_path, true)
        } else if crate_relative_path.exists() {
            (crate_relative_path, true)
        } else {
        ...
        if exists {
            ...
            println!(
                "Project file not found: {file} (tried both workspace and crate-relative paths)"
            );
        }
    }
...
fn test_rule_specific_cases() {
    ...
    // Rule-specific tests should have reasonable accuracy
    // Note: MDBOOK* rules naturally have 0% compatibility with markdownlint
    // since they're mdBook-specific extensions
    assert!(
        report.success_percentage() >= 30.0,
